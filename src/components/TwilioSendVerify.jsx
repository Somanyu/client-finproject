    ...
    <div>
      <div className="flex items-center flex-shrink-0">
        <button
          onClick={handleClick}
          type="button"
          className={`text-white font-roboto bg-teal-700 hover:bg-teal-800 focus:ring-4 focus:outline-none focus:ring-teal-300 font-medium rounded-lg text-sm px-5 py-2 text-center mr-2 dark:bg-teal-600 dark:hover:bg-teal-700 dark:focus:ring-teal-800 inline-flex items-center ${loading ? 'cursor-not-allowed opacity-50' : ''}`}
          disabled={loading}
        >
          {loading && (
            <svg
          ...
          {loading ? 'Loading...' : status === 'sent' ? 'Sent' : status === 'failed' ? 'Failed' : 'Call API'}
        </button>
